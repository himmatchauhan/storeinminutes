...
window['isTouchSupported'] = ('ontouchstart' in window);
jQuery(function($) {
    FindDuplicateIDs_debug();
    setTimeout(function() {
        var $globalPageHeader = $('.global-page-header .global-page-header-h1');
        var $globalPageHeaderItem = $('.global-page-header .global-page-header-title.title-item .item');
        var $globalPageHeaderSmallItem = $('.global-page-header .global-page-header-title.title-item .item h1.small');
        if ($globalPageHeader.length === 0 || $globalPageHeaderItem.length === 0) return;
        var headerFontSize = parseFloat($globalPageHeader.css('font-size'));
        var smallItemWidth = $globalPageHeaderSmallItem.length > 0 ? $globalPageHeaderSmallItem.width() + 28 : 0;
        while ($globalPageHeader.width() > ($globalPageHeaderItem.width() - smallItemWidth) && headerFontSize > 12) {
            headerFontSize--;
            $globalPageHeader.css('font-size', `${headerFontSize}px`);
        }
        if ($globalPageHeader.width() > $globalPageHeaderItem.width()) {
            $globalPageHeader.css('text-overflow', 'ellipsis');
            $globalPageHeader.css('overflow', 'hidden');
        }
        $globalPageHeader.css('visibility', 'visible');
    }, 50);
    SearchDomain.init();
    designSystemActions();
    InitializeBackgroundLazyload();
...
function designSystemActions() {
    ...
    $(".popup_alert").click(function(event) {
        ...
        if ($this.attr('data-open-swal-popup') && $this.attr('data-open-swal-popup') == 'false') return;
        if ($this.data('swal-dialog-type') && $this.data('swal-dialog-type') == 'confirm') {
            let confirmButtonClass = 'btn btn-success';
            let cancelButtonClass = 'btn btn-danger';
            let confirmButtonText = translations.yes;
            let cancelButtonText = translations.no;
            let callback = null;
            if ($this.data('swal-confirm-button-class') && $this.data('swal-confirm-button-class').length > 0) {
                confirmButtonClass = $this.data('swal-confirm-button-class');
            }
            if ($this.data('swal-cancel-button-class') && $this.data('swal-cancel-button-class').length > 0) {
                cancelButtonClass = $this.data('swal-cancel-button-class');
            }
            if ($this.data('swal-confirm-button-text') && $this.data('swal-confirm-button-text').length > 0) {
                confirmButtonText = $this.data('swal-confirm-button-text');
            }
            if ($this.data('swal-cancel-button-text') && $this.data('swal-cancel-button-text').length > 0) {
                cancelButtonText = $this.data('swal-cancel-button-text');
            }
            if ($this.data('swal-callback') && $this.data('swal-callback').length > 0) {
                callback = $this.data('swal-callback');
            }
            let customSwal = Swal.mixin({
                customClass: {
                    confirmButton: confirmButtonClass,
                    cancelButton: cancelButtonClass,
                    actions: 'grid-system xl-grid-2',
                },
                buttonsStyling: false
            });
            if ($this.data('open-swal-popup')) {
                $this.attr('data-open-swal-popup', 'false');
            }
            customSwal.fire({
                title: $this.data('swal-title'),
                text: $this.data('swal-text'),
                icon: $this.data('swal-icon'),
    ...
    InitializeQuickCopy($('.uiQuickCopy'));
    $('.alert .alert-minimize-content-expand').on('click', function(e) {
        e.preventDefault(); // Prevent any default button behavior
        var $content = $(this).siblings('.alert-minimize-content');
...
function FindDuplicateIDs_debug() {
    if (typeof $GLOBALS !== 'undefined' && $GLOBALS.is_local_server) {
        var ids = {};
        var duplicateIDs = $('[id]').map(function() {
            var id = $(this).attr('id');
            if (ids[id]) {
                return id; // This is a duplicate
            }
            ids[id] = true;
            return null;
        }).get().filter(Boolean);
        if (duplicateIDs.length > 0) {
            Swal.fire({
                title: 'Duplicate IDs found: ' + duplicateIDs.join(', '),
                icon: 'warning'
            });
        }
    }
            ...
            $.get('/manager/domain/checkDomainAvailabilityReturnHTML.php', {
                ...
                if (SearchDomainSettings.insideWizard) {
                    $('#domainMoreOptions').on('show.bs.modal', function(event) {
                        ...
                        var heighestHeightNUM = window['isTouchSupported'] ? $(window).outerHeight(true) - 300 : $(window).outerHeight(true) - 200;
...
function InitializeBackgroundLazyload() {
    new LazyLoad({
        elements_selector: ".bgLazyload",
        callback_enter: function(el) {}
    });
}

function InitializeQuickCopy($uiQuickCopy) {
    $uiQuickCopy.each(function() {
        let $this = $(this);
        let el = $(this).data('copy-id');
        let $el = $('#' + el);
        if ($el.attr('readonly') && !$el.hasClass('qc-input-not-copied')) {
